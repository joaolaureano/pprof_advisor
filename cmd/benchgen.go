...
func newBenchgenCmd() *cobra.Command {
	...
	c := &cobra.Command{
		...
		Long: "Generate a same-package fuzz test and benchmark without a model or API key. " +
			...
			"An argument may be an interface as long as the target package declares a concrete type that satisfies it and is itself composed of fuzz types; " +
			"--impl Interface=Type picks a concrete implementation when several qualify. The benchmark measures the chosen implementation, not the interface. " +
	...
	f.StringArrayVar(&opts.Implementations, "impl", nil, "concrete type to instantiate for an interface parameter, as Interface=Type; repeatable")
